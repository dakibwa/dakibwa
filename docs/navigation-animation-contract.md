...
- Project cards share one image-above-caption anatomy. Hover or keyboard focus previews a short serif description and an underlined destination link below the row; click/tap holds it open. Keep this disclosure on the page background, without a box, repeated title, category heading or filled button. Escape, another selection and leaving the section by keyboard dismiss it. It expands in normal flow over 340ms, physically moving the Career divider. Keep the last content mounted during closing so the divider returns smoothly; the closed panel is inert and hidden from assistive technology. On phones, projects form a native horizontal snap rail, with the shared disclosure outside the clipped rail and its link beneath the description.
