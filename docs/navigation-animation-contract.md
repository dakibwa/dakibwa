...
- Career keeps the original coloured node line, logo cards, small serif mission text and selective bold emphasis. A quiet line shows the current role from the approved curation; dates remain visible beneath every logo. Its 280px detail (260px on phones) occupies real layout space and leaves 24–36px before the Taste divider. Keyboard, tap-to-hold, hover and Escape remain available. Narrow phones scroll the timeline natively with enough card width to read the dates.
...
- Career and Taste share the project reveal's 480ms opening and 340ms closing rhythm. Measure the text so both the box and its space adjust smoothly to each item, without a fixed-height spacer. On handover, glide the existing box beneath the new item, dissolve the previous text over 140ms and settle the incoming text over 320ms. Rapid changes interrupt cleanly and finish with the latest selection. Follow actual card/rail bounds on scroll and resize, clamp the readable box inside the page, and dismiss it when its card leaves view. Keep the last text mounted through closing; closed boxes are inert and hidden from assistive technology, and outgoing text is never announced twice. Reduced motion disables geometry, travel and text animation.
- The homepage Taste archive is a native horizontal rail with coloured word navigation and a twelve-cover mixed edit. Desktop covers reveal their title, creator and available play count in one shared panel beneath the rail on hover or keyboard focus. Escape, leaving the section, changing category or scrolling the active cover out of view dismisses it. Touch devices keep the title, creator and count visible beneath each cover.
- Taste's compact Search control opens a field for titles and creators. Search inside Highlights covers the whole library; a selected medium limits the search to that medium. Match words without case or accent sensitivity. Keep the query when changing medium, show a readable empty state, and restore the twelve-cover edit when search is cleared or closed. Escape closes search and returns focus to its control.
