...
- Career keeps the original coloured node line, logo cards, small serif mission text and selective bold emphasis. Dates remain visible beneath every logo; the extra current-role line above the timeline is removed. Its 280px detail (260px on phones) occupies real layout space and leaves 24–36px before the Taste divider. Keyboard, tap-to-hold, hover and Escape remain available. Narrow phones scroll the timeline natively with enough card width to read the dates.
...
- Projects, Career and Taste use the same measured reveal, with a 480ms opening and 340ms closing rhythm. Projects and Career make real space for the box. On handover, animate the existing box's horizontal position, width and height instead of jumping its margin; dissolve the previous text over 140ms and settle the incoming text over 320ms. Project boxes continue to match the visible card width. Rapid changes interrupt cleanly and finish with the latest selection. Follow actual card/rail bounds on scroll and resize, clamp the readable box inside the page, and dismiss it when its card leaves view. Keep the last text mounted through closing; closed boxes are inert and hidden from assistive technology, and outgoing text is never announced twice. Reduced motion disables geometry, travel and text animation.
- The homepage Taste archive is a dense wall of artwork with coloured word navigation and a forty-eight-cover mixed edit. Stack three or four pieces vertically, using their natural portrait or square proportions, narrow consistent gaps and gently staggered column tops. Preserve catalogue order down each stack and then across the wall; never reorder listening ranks just to fill a gap. The wall scrolls horizontally with the existing arrows, native swiping and keyboard focus. Let shorter search results occupy their natural height.
- Desktop covers reveal title, creator and available play count in one shared floating panel next to the selected artwork. It glides both across columns and between rows, fits above or below the cover according to viewport space, and leaves the selected artwork visible without shifting the wall. The noninteractive panel never intercepts movement onto another cover. Escape, leaving the section, changing category or scrolling the active cover out of view dismisses it; artwork moving under a stationary pointer must not reopen it. Touch devices keep title, creator and count visible beneath each cover, with enough space between stacked cards.
- Keep the Taste heading, Search control and browsing arrows on one horizontal line. On phones, use compact icon controls and scale the heading to fit. Opening search replaces the visible phone heading with the field in that same row, while retaining the heading for assistive technology. Closing search restores it without adding another header row.
- Taste's compact Search control opens a field for titles and creators. Search inside Highlights covers the whole library; a selected medium limits the search to that medium. Match words without case or accent sensitivity. Keep the query when changing medium, show a readable empty state, and restore the mixed wall when search is cleared or closed. Escape closes search and returns focus to its control.
