...
- Projects, Career and Taste use the same measured reveal, with a 480ms opening and 340ms closing rhythm. Projects makes real space for the box; Career reserves space when its floating box extends below the timeline. On handover, animate the existing box's horizontal position, width and height instead of jumping its margin; dissolve the previous text over 140ms and settle the incoming text over 320ms. Project boxes continue to match the visible card width. Rapid changes interrupt cleanly and finish with the latest selection. Follow actual card/rail bounds on scroll and resize, clamp the readable box inside the page, and dismiss it when its card leaves view. Keep the last text mounted through closing; closed boxes are inert and hidden from assistive technology, and outgoing text is never announced twice. Reduced motion disables geometry, travel and text animation.
...
- Desktop covers reveal title, creator and available play count in one shared floating panel next to the selected artwork. It glides both across columns and between rows, varying among above, below, left and right placements. Give each item a stable preference, then use the available viewport space while keeping the selected artwork and chapter controls clear; do not choose a fresh random position on every hover. The wall itself stays still. The noninteractive panel never intercepts movement onto another cover. Escape, leaving the section, changing category or scrolling the active cover out of view dismisses it; artwork moving under a stationary pointer must not reopen it. Touch devices keep title, creator and count visible beneath each cover, with enough space between stacked cards.
