...
`/trek/` is a static surface at `public/trek/index.html` — the walk travelled through the map. One full-screen atlas follows the GPS day points through 17 city anchors from Paris to Sofia; scroll it yourself or press **Begin the journey**. The cached 56 × 34 Open-Meteo elevation grid in `data/trek-terrain.json` gives the land real relief, while selected Natural Earth lakes and rivers keep the geography legible without a mapping runtime. The terrain runs full-bleed beneath translucent story, statistics and collection panels; decorative landmark and mountain doodles stay removed so the route, places and real material from the road do the storytelling. The pinned masthead and stat bar build distance, days on foot, ascent, moving time and live map elevation, with flat colour-coded cell meters showing each measure against its journey maximum. The active country colours the route and territory, and the right-hand desktop collector accumulates notable places and albums. Each border crossing opens a restrained country plate. Rest-day points remain on the route but pass silently.
