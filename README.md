...
`/trek/` is a static surface at `public/trek/index.html`. Same walk, Features grammar: coloured country threads through the 67 numbered days. Walked-day nodes are tiny record sleeves on that thread (Cover Art Archive fronts where the GPS title equals a MusicBrainz album/EP; unmatched days stay blank jackets, rest days stay empty pins). The life-map GPS sits behind as quiet contour. About 38 km on a walked day (1,982 km / 52). Build with `npm run trek:covers` then `npm run trek:build`. The wall’s Trek card and the walk chapter on `/life-map/` both open it.
