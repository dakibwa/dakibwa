...
const links=read('public/trek/route-links.json'),profile=read('public/trek/elevation-profile.json');
...
const metrics=Metrics.create(Route.buildJourneyPath(route,67,links),links,profile,days);
for(const day of days){const estimate=metrics.day(day.n);Object.assign(day,{estimatedKm:estimate.km,estimatedAscent:estimate.ascent});}
...
const assets=Object.fromEntries(['vendor/maplibre-gl.js','route-detail.json','route-links.json','moments.json','journey-style.json','elevation-profile.json'].map(file=>[file,createHash('sha256').update(readFileSync(new URL('public/trek/'+file,root))).digest('hex').slice(0,12)]));
const data={assets,landmarks,countryRings:source.countryRings.map(({name,rings})=>({name,rings})),days,photos,scenes,timeline,total:metrics.total.km,recorded:metrics.recorded,estimated:metrics.estimated,colors:Object.fromEntries(source.countries.map(c=>[c.name,c.color])),stats:{days:walked,ascent:metrics.total.ascent,minutes,countries:source.facts.countries}};
const esc=s=>String(s).replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
const list=days.map(d=>`<li>Day ${d.n} · ${esc(d.c)}${d.date?' · '+d.date:''}${d.w?' · '+d.km.toFixed(1)+' km recorded':''}${d.estimatedKm>0?' · '+d.estimatedKm.toFixed(1)+' km and '+Math.round(d.estimatedAscent)+' m ascent estimated':''}${d.j.length?'<p>'+esc(d.j.map(j=>j.text).join(' '))+'</p>':''}</li>`).join('\n');
const html=readFileSync(new URL('scripts/trek-journey-template.html',root),'utf8').replace('__DATA_JSON__',JSON.stringify(data)).replace('<!--__NOSCRIPT_DAYS__-->',list)
  .replaceAll('__TOTAL_KM__',Math.round(data.total).toLocaleString('en-GB'))
