...
export async function checkTrekPaths({cdp,evaluate,goto,setDesktop,sleep,check,section,capture}){
  ...
  if(process.env.CHECK_TREK_VEHICLES_ONLY==='1'){
    ...
    const pixelCount=async(kind,position)=>{
      const {data}=await cdp.send('Page.captureScreenshot',{format:'png'}),sharp=require('sharp');
      const {data:pixels,info}=await sharp(Buffer.from(data,'base64')).removeAlpha().raw().toBuffer({resolveWithObject:true});
      const {screen}=position;let count=0;
      for(const [cx,cy] of screen)for(let y=Math.max(0,Math.floor(cy)-60);y<Math.min(info.height,cy+20);y++)for(let x=Math.max(0,Math.floor(cx)-32);x<Math.min(info.width,cx+32);x++){
        const i=(y*info.width+x)*info.channels,[r,g,b]=pixels.slice(i,i+3);
        if(kind==='train'?r>80&&r>g*1.7&&b>g*.7:b>r*1.3&&g>r*1.3&&g<150)count++;
      }
      return count;
    };
    ...
    for(const [width,height] of [[1440,900],[390,844]]){
      ...
      for(const [index,tunnel] of tunnels.entries()){
        await scrub((tunnel.start+tunnel.end)/2);check(await prepared(),'the mapped tunnel prepares');
        const s=await state();check(s.train.cars.length===3&&s.train.cars.some(c=>c.tunnel)&&s.train.vertices>1000&&!s.train.failed,'all coaches remain rendered inside the mapped tunnel');
        check(await pixelCount('train',s.train)>15,'the train paint is visible in the actual rendered tunnel view');
        await capture?.(`trek-tunnel-${index}-${width}`);
        if(width===390&&index>0)continue;
        await scrub(tunnel.start-100);check(await prepared(),'the tunnel entrance prepares');const before=await totals();
        await evaluate(`window.__vehicleFrames=[];window.__vehicleCapture=true;document.querySelector('#play').click();requestAnimationFrame(function sample(time){if(!window.__vehicleCapture)return;const s=window.trekStatus();window.__vehicleFrames.push({time,distance:s.distance,train:s.train,visible:s.routeVisible});requestAnimationFrame(sample);});`);
        check(await until(async()=>(await state()).distance>tunnel.end+100,25000),'playback crosses the whole tunnel');
        const frames=await evaluate(`(()=>{document.querySelector('#play').click();window.__vehicleCapture=false;const f=window.__vehicleFrames;delete window.__vehicleFrames;delete window.__vehicleCapture;return f;})()`);
        check(frames.length>50&&frames.every(f=>f.train.cars.length===3&&f.train.opacity===1),'no train frame disappears at either tunnel boundary');
