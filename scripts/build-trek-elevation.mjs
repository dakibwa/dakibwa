...
const profile={version:2,source:'Mapzen terrain tiles',sourceUrl:'https://www.mapzen.com/rights/',method:'Mapped ground elevation across every recorded path and presentation connection, sampled about every 200 m; not recorded GPS altitude. Uphill changes along reconstructed walking paths provide estimated ascent; train connection heights add no walking ascent.',zoom,step,routeHash:createHash('sha256').update(routeBytes).digest('hex'),linksHash:createHash('sha256').update(linksBytes).digest('hex'),total:Math.round(path.total),min:Math.min(...heights),max:Math.max(...heights),pieces};
