...
test("a permanently wrong artifact fails at the deadline with expected and actual hashes", async () => {
  ...
  await fixture((_request, response) => {
    ...
    await assert.rejects(waitForHostedExport(origin, hash(expected), { timeoutMs: 80, intervalMs: 10, log: quiet }), (error) => {
...
test("matching bytes with a failure status never count as the deployed page", async () => {
  await fixture((_request, response) => {
    ...
    await assert.rejects(waitForHostedExport(origin, hash(expected), { timeoutMs: 80, intervalMs: 10, log: quiet }), /HTTP 503/);
  });
});
...
test("a stalled response cannot extend the readiness deadline by a request timeout", async () => {
  await fixture(() => {}, async (origin) => {
    ...
    await assert.rejects(waitForHostedExport(origin, hash(expected), { timeoutMs: 80, intervalMs: 10, log: quiet }), /did not become ready/);
    assert.ok(performance.now() - start < 1000, "a stalled request must use the remaining readiness budget");
  });
});
